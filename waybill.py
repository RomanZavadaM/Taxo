...
написи будуються заново. Taxo підставляє відомі планові дані; порожні
клітинки залишаються для фактичних відміток під час рейсу.
"""
...
def _page_one(c, data):
    ...
    # Пробіг / рейси / паливо, графи А та 37-62.
    y = y + header_h + 85
...
def build_waybill_pdf(template_path, output_path, data):
    ...
    c.setTitle(f"Шляхівка {data.get('waybill_no','')}"); c.setAuthor("Taxo")
