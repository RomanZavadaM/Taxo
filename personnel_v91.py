...
def export_p5_pdf(core, year, month, out_path, active_only=True):
    ...
    legend = []
    seen = set()
    for _dtype, (letter, numeric, label) in P5_CODES.items():
        key = (letter, numeric, label)
        if letter and key not in seen:
            seen.add(key)
            legend.append(key)
    col_gap = 20
    x0 = 24
...
def export_p5_xlsx(core, year, month, out_path, active_only=True):
    ...
    for _dtype, (letter, numeric, label) in P5_CODES.items():
        ...
        if not letter or key in seen:
            continue
        seen.add(key); legend_ws.append([label, letter, numeric])
