...
class TestTaxo100StableRelease(unittest.TestCase):
    def test_current_version_is_stable_10(self):
        self.assertEqual(version_from_file(ROOT / "VERSION.txt"), "10.0")
        text = (ROOT / "VERSION.txt").read_text("utf-8")
        self.assertIn("Version: 10.0", text)
        self.assertIn("Release type: stable", text)
        self.assertIn("Previous stable / rollback point: Taxo 9.0.1", text)

    def test_runtime_version_markers_are_10(self):
        self.assertEqual(v91_features.APP_VERSION, "10.0")
        self.assertEqual(personnel_v91.APP_VERSION, "10.0")
        self.assertIn("Taxo 10.0", v91_features.WINDOW_TITLE)
        self.assertIn("Taxo 10.0", personnel_v91.WINDOW_TITLE)
        self.assertIn("Taxo 10.0 stable", (ROOT / "taxo_app.py").read_text("utf-8"))
