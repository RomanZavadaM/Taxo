...
> **Current prerelease:** Taxo 10.3-r2 — плановий бланк зберігається, а реальні відхилення записуються окремим фактичним шаром без перезапису плану.  
...
Незмінним stable release залишається **Taxo 10.1 (`v10.1`)**. Робочий candidate **10.3-r2** побудована від прийнятого checkpoint 10.2-r7 і проходить candidate-перевірку. Робочі БД, скани, кеші та персональні документи не входять до GitHub-релізів; оновлення програми не потребує повторного введення робочої бази.
...
### Taxo 10.3-r1 — контроль Бланків після зміни ТАХО/графіка
...
### Taxo 10.3-r1 — межі відпочинку за повною робочою зміною
...
### Taxo 10.3-r2 — план і факт бланків

Початкова логіка збережена: **«ПОТОЧНИЙ — ПІДГОТУВАТИ»** дозволяє заздалегідь сформувати бланк за плановим часом. У більшості випадків план автоматично є фактом і додаткового введення не потрібно.

Якщо фактична ситуація відрізняється (ранній/пізній виїзд, пізніше повернення, несправність, хвороба посеред зміни тощо), редагується підготовлений бланк. Taxo зберігає початковий план і записує лише фактичну поправку `fact_work_*`. Для табеля та фактичних реєстрів використовується факт, якщо він є; інакше — план.

Бланки 14/15/16 можуть коригувати фактичні межі робочого часу. Позиції 17/18/19 не вважаються відпочинком і автоматично не обрізають роботу. Якщо за період було кілька причин, вони оформлюються окремими послідовними бланками.
