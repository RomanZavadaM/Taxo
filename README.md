...
> **Current prerelease:** Taxo 10.3-r2 — єдиний маршрут планування персоналу; водійський графік лишається спеціалізованим, а оперативний випуск більше не є другим планувальником.  
...
Незмінним stable release залишається **Taxo 10.1 (`v10.1`)**. Робоча гілка **10.2-r10** побудована від прийнятого checkpoint 10.2-r7 і проходить candidate-перевірку. Робочі БД, скани, кеші та персональні документи не входять до GitHub-релізів; оновлення програми не потребує повторного введення робочої бази.
...
### Taxo 10.3-r2 — контроль Бланків після зміни ТАХО/графіка

Якщо після уточнення фактичного часу активний Бланк підтвердження перекриває лише частину потрібного періоду, Taxo не створює додатковий короткий бланк на залишок. Контроль пропонує уточнити межі існуючого документа новою ревізією; попередня версія і файли зберігаються в архіві.


### Taxo 10.3-r2 — межі відпочинку за повною робочою зміною

Час маршруту/тахокарти не дорівнює повному робочому часу водія. Перед виїздом і після повернення можуть бути підготовчі роботи, оформлення документів, медогляд та інші обов'язки, які не завжди видно на тахокарті.

Контроль Бланків використовує `work_start_time/work_end_time`. Якщо фактичний час керування зміщується, індивідуальний перед-/післямаршрутний запас із сценарію маршруту переноситься разом із ним. Якщо користувач вручну змінює межі існуючого Бланка, Taxo автоматично коригує суміжний робочий час: початок бланка = кінець попередньої роботи, кінець бланка = початок наступної. Час керування при цьому не переписується.


### Taxo 10.3-r2 — Підготовка Бланка наперед і фактичне уточнення

Бланк підтвердження можна готувати завчасно за плановим часом наступного виїзду. Це робочий попередній варіант для ранкового випуску.

Якщо фактичний виїзд/повернення або причина відсутності відрізняються від плану, той самий Бланк уточнюється новою ревізією. Окремі короткі документи на різницю в часі не створюються. Зміна меж Бланка не змінює ні фактичний робочий час, ні інтервал керування. Табель і зарплата рахуються за окремо підтвердженим фактом роботи.


### Бланк не є джерелом оплати праці

Межі Бланка підтвердження можуть відрізнятися від фактичних меж робочої зміни. Це допустимо: Бланк підтверджує вид діяльності/відсутності для контролю, а табель і зарплата повинні враховувати весь реально відпрацьований час.

Редагування Бланка не переписує `work_start_time`, `work_end_time`, `work_hours` або час керування. Розбіжність між Бланком, графіком, ТАХО та фактичним робочим часом може контролюватися окремо, але не виправляється приховано.
